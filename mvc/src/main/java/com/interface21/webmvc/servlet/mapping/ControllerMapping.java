package com.interface21.webmvc.servlet.mapping;

import com.interface21.webmvc.servlet.Handler;
import com.interface21.webmvc.servlet.HandlerType;
import com.interface21.webmvc.servlet.mvc.asis.Controller;
import jakarta.servlet.http.HttpServletRequest;
import java.util.Map;
import lombok.AccessLevel;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;

@Slf4j
@RequiredArgsConstructor(access = AccessLevel.PRIVATE)
public class ControllerMapping implements HandlerMapping {

    private final Map<String, Controller> controllers;

    public static ControllerMapping from(final Map<String, Controller> controllers) {
        log.info("Initialized Handler Mapping!");
        controllers.keySet()
                .forEach(path -> log.info("Path : {}, Controller : {}", path, controllers.get(path).getClass()));

        return new ControllerMapping(controllers);
    }

    @Override
    public Handler getHandler(final HttpServletRequest request) {
        final String requestURI = request.getRequestURI();
        log.debug("Request Mapping Uri : {}", requestURI);
        final Controller controller = controllers.get(requestURI);
        return new Handler(controller, HandlerType.CONTROLLER);
    }
}
