...
class JsonViewResolverTest {
    ...
    private static Stream<Arguments> jsonViewResolver_resolve() {
        return Stream.of(
                ...
                Arguments.of("view가 .jsp로 끝나는 String일 경우 null을 반환한다", "index.jsp", false),
                Arguments.of("view가 redirect:로 시작하는 String일 경우 null을 반환한다", "redirect:/index.jsp", false),
                Arguments.of("view가 null이 아니고 redirect:로 시작하거나 .jsp로 끝나는 String이 아니면 JsonView를 반환한다",
                        new User(1L, "gugu", "hkkang@woowahan.com"), true)
        );
    }
}
