...
class ReflectionTest {
    ...
    void givenObject_whenGetsClassName_thenCorrect() {
        ...
        assertThat(clazz.getSimpleName()).isEqualTo("Question"); // ClassName을 반환
        assertThat(clazz.getName()).isEqualTo("reflection.Question"); // 패키지 경로와 함께 이름을 반환, 배열과 같은 것은 정해진 이름을 반환
        assertThat(clazz.getCanonicalName()).isEqualTo(
                "reflection.Question"); // 일반 클래스의 경우 패키지 경로와 함께 이름반환, 배열과같은 것은 사용자가 보기 쉽게 이름반환
    }
    ...
    void givenObject_whenGetsFieldNamesAtRuntime_thenCorrect() {
        ...
        final Field[] fields = student.getClass().getDeclaredFields(); // 이것 또한, decalared를 붙여야 private까지 가져온다.
